...
std::tuple<int, int64_t> gds_extract(gds_db* db, const char* cell_name, gds_bbox target, int64_t resolution, gds_polyset* pset)
{
	...
	{
		printf("\n--> Cell name not found\n");
		return std::make_tuple(GDS_ERR_CELL_NAME_NOT_FOUND, 0);
	}
	...
	{
		printf("--> %s\n", info.error);
		return std::make_tuple(GDS_ERR_MAX_POLYS, info.nskipped);
	}

	if (info.pset->size() == 0)
	{
		printf("--> No polygons found\n");
		return std::make_tuple(GDS_ERR_NO_POLYS_FOUND, info.nskipped);
	} else
	{
		printf("--> Found %lld polygons\n", info.pset->size());
	}

	return std::make_tuple(GDS_ERR_SUCCESS, info.nskipped);
}
